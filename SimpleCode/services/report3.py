...
def aggregate_report_app(logs, balances):
    # fn = lambda advert, app: app[advert["payment_type"]]["sharing"]
    fn = lambda advert, app: float(balances.get("%s-%s-share" % (advert["_id"], app["_id"]), 0))
    __ = lambda x: x[0]
    lst = []
    details = []
...
def aggregate_report_by_campaign(logs, fn_base):
    for app_id, lst1 in groupby(sorted(logs.keys(), key=k_app), k_app):
        lst1 = list(lst1)
        ...
        for advert_id, lst2 in groupby(sorted(lst1, key=k_advert), k_advert):
            ...
            #                      % advert["payment_type"])
            report = summarize_advert(lst2, logs)
            report["balance"] = fn_base(advert, app)
            yield (report, advert, app)



def aggregate_report_advert(logs, balances):
    ret = []
    for __, lst in groupby(sorted(logs.keys(), key=k_advert), k_advert):
        lst = list(lst)
        advert = get_advert_and_transform(__)
        report = summarize_advert(lst, logs)
        report["type"] = advert["payment_type"]
        report["balance"] = float(balances.get("%s-cost" % advert["_id"], 0))
        ret.append((__, report))

    return ret

	
def summarize_advert(lst, logs):
    d = {}
    d["requests"] = d["impressions"] = d["clicks"] = d["actions"] = 0
    for key in lst:
        action_code = key.split("-")[-1]
        count = int(logs.get(key, 0))
...
def aggregate_report_platform(logs, balances):
    #fn = lambda ad, app: ad["price"] - app[ad["payment_type"]]["sharing"]
    fn = lambda ad, app: float(balances.get("%s-%s-benefit" % (ad["_id"], app["_id"]), 0))
    __ = lambda x: x[0]
    lst = []
    for (report, advert, app) in aggregate_report_by_campaign(logs, fn):
...
def aggregate_all(logs, balances, date_start, date_end):
    ret = []
    # --------------------
    # Platform Report
    buffers = []
    ...
    total["actions"] = []
    total["revenue"] = []
    for (group_name, report) in aggregate_report_platform(logs, balances):
        d = ReportPlatformLog()
    ...
    # Advert Report
    buffers = []
    for (advert_id, report) in aggregate_report_advert(logs, balances):
        d = ReportAdvertLog()
    ...
    reports, details = aggregate_report_app(logs, balances)
    buffers = []
...
def upsert_report_by_period3(start_time, end_time):
    ...
    # get track log and balance at one time
    logs, balances = redis_conn.pipeline().hgetall("periodical_log_%s" % timestamp) \
        .hgetall("periodical_balance_%s" % timestamp) \
        .execute()
    # hkey = "periodical_log_%s" % timestamp
    # logs = redis_conn.hkeys(hkey)

    buffers = aggregate_all(logs, balances, start_time, end_time)
...
def upsert_dsp_report_by_period(start_time, end_time):
    timestamp = start_time.strftime("%Y%m%d%H")
    # get track log and balance at one time
    logs, balances = redis_conn.pipeline().hgetall("dsp_periodical_log_%s" % timestamp) \
        .hgetall("dsp_periodical_balance_%s" % timestamp) \
        .execute()
    #hkey = "dsp_periodical_log_%s" % start_time.strftime("%Y%m%d%H")
    #logs = redis_conn.hkeys(hkey)

    buffers = dsp_aggregate_all(logs, balances, start_time, end_time)
    now = datetime.datetime.utcnow()
...
def dsp_aggregate_all(logs, balances, date_start, date_end):
    ret = []

    buffers = []
    reports, details = dsp_aggregate_report_app(logs, balances)
...
def dsp_aggregate_report_app(logs, balances):
    fn = lambda dsp, show_type, app: float(balances.get("%s-%s-%s-share" % (dsp, show_type, app["_id"]), 0))
    __ = lambda x: "{0}-{1}".format(*x)
    details = []
    for (report, show_type, dsp, app) in dsp_aggregate_report_by_campaign(logs, fn):
        report["cpm_balance"] = report["cpc_balance"] = report["cpa_balance"] = 0
...
def dsp_aggregate_report_by_campaign(logs, fn_base):
    for dsp_app_id, lst1 in groupby(sorted(logs, key=k_dsp_app_show_type), k_dsp_app_show_type):
        ...
        app = get_app_and_transform(app_id)
        report  = dsp_summarize_advert(lst1, logs)
        report["balance"] = fn_base(dsp, show_type, app)
        yield (report, show_type, dsp, app)

def dsp_summarize_advert(lst, logs):
    d = {}
    d["requests"] = d["impressions"] = d["clicks"] = d["actions"] = 0

    for key in lst:
        action_code = key.split("-")[-2]
        count = int(logs.get(key, 0))
